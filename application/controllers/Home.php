...
class Home extends CI_Controller {
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "") {
        ...
        if($page_data['lesson_id']) $page_data['video_url'] = $this->ajax_model->get_video_url_by_lesson_id($page_data['lesson_id']);
        $page_data['course_id']  = $course_id;
        $page_data['page_name']  = 'lessons';
        $page_data['page_title'] = $course_details['title'];
        $page_data['is_free_course'] = $this->forum_model->get_course_by_id($course_id)->is_free_course;

        $this->load->view('lessons/index', $page_data);

        if(!$page_data['is_free_course']) {
            $qa_data = [
                'course_id' => $course_id,
                'user_id' => $this->session->user_id,
                'question_count' => 5
            ];
            if($this->input->get('question_id')) $qa_data['question_id'] = $this->input->get('question_id');
            $this->load->view('forum/lesson_question', $qa_data);
        }
    }
