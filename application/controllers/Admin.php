...
class Admin extends CI_Controller {
  ...
  public function instructor_term($user_id) {
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		if($user_id == null) {
		...
		$page_data['page_name'] = 'instructor_term';
		$page_data['page_title'] = 'Instructor Term';
		$page_data['instructor'] = $this->ajax_model->get_instructor_by_user_id($user_id);
		$this->load->view('backend/index', $page_data);
  }
  ...
  public function instructor_revenue($param1 = "") {
		...
		$payment_history = $this->crud_model->get_revenue_by_user_type("", "", 'instructor_revenue');
		for($i = count($payment_history)-1; $i >= 0; $i--) {
			$payment_history[$i]['file_url'] = $this->ajax_model->get_file_url('pdf', $payment_history[$i]['attachment_name']);
		}
		$page_data['payment_history'] = $payment_history;
